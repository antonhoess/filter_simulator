...
def test_1():
    class Config(ConfigSettings):
        def __init__(self, obj=None):
            ...
            self._add_attribute("a", float)

            if obj:
                self._copy_attributes(obj)
            # end if
        # end def
    # end class

    class ConfigSub1(Config):
        def __init__(self):  # , obj=None):
            super().__init__()
            self._add_attribute("log", int)
            self._add_attribute("o", object, nullable=True)
            self._add_attribute("x", X1)
            self._add_attribute("l", str, islist=True)
            self._add_attribute("two", [int, str])
            self._add_attribute("twolist", [int, str], islist=True)
        # end def
    # end class
    ...
    setattr(b, "a", 5.7)
    setattr(b, "log", -2)
    setattr(b, "o", None)  # object())
    setattr(b, "x", X2())
    setattr(b, "s", "asdf")
    setattr(b, "l", ["a", ""])
    setattr(b, "two", 3)
    setattr(b, "twolist", [1, "", 3.4])

    c1 = ConfigSub1.from_obj(b)
...
def test_2():
    ...
    class BaseFilterSimulatorConfigSettings(ConfigSettings):
        #     # General group
        #     fov: float
        fov = None
        ...
        def __init__(self):
            ...
            # General group
            self._add_attribute("fov", float)
        # end def
    # end def

    class BaseFilterSimulatorConfigSettings2(BaseFilterSimulatorConfigSettings):
        xx = None
    ...
    args = Tmp()
    args.fov = 5
    c = ConfigSettings.from_obj(args)

    cx2 = BaseFilterSimulatorConfigSettings2()
    cx2.fov = -3.14
    cx2.xx = "test"
    cx = BaseFilterSimulatorConfigSettings.from_obj(cx2)
